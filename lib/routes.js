...
module.exports = function (app, addon) {
  ...
  router.post('/config', addon.authenticate(), function (req, res) {
    ...
    return sauceAccount.getAccountDetailsAsync().then(() => {
      return addon.settings.set('sauceAccount', data, req.clientInfo.clientKey).then(() => {
        return getGlanceData(req.clientInfo.clientKey)
          .then(data => hipchat.updateGlance(req.clientInfo, req.identity.roomId, 'saucelabs.glance', data));
      })
    }).catch(err => {
  ...
  router.post('/webhooks/sauce', addon.authenticate(), function(req, res) {
    return getGlanceData(req.clientInfo.clientKey)
      .then(data => hipchat.updateGlance(req.clientInfo, req.identity.roomId, 'saucelabs.glance', data))
      .then(() => hipchat.sendMessage(req.clientInfo, req.identity.roomId, 'Updating glance'))
    ...
    return addon.settings.getAllClientInfos().then(clients => {
      return clients.map(clientInfo => {
        return getGlanceData(clientInfo.clientKey)
          ...
          .catch(err => {
            console.error('updateAllGlances error: ', err);
