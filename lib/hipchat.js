...
var {extend, pickBy} = require('lodash');
...
module.exports = function(addon) {
      ...
      function makeRequest(clientInfo) {
        addon.getAccessToken(clientInfo).then(function(token){
          ...
          http({
            ...
            qs: extend({auth_token: token.access_token}, options.qs),
            body: options.body,
            json: true
  ...
  return {
    ...
    getEmoticons: function (clientInfo, start=0, limit=1000) {
      return request(clientInfo, {
        method: 'GET',
        resource: '/emoticon',
        qs: {'start-index': start, 'max-results': limit}
      });
    },
    ...
    updateGlance: function (clientInfo, roomId, moduleKey, glance) {
      return request(clientInfo, {
        method: 'POST',
        resource: roomId ? '/addon/ui/room/' + roomId : '/addon/ui',
        body: {
          "glance": [{
            "key": moduleKey,
            "content": glance
