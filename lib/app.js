...
var addon = ac(app, null, null, function() { addon.emit('configured'); });
app.set('addon', addon);
...
hbs.registerHelper('ROLLBAR_CLIENT_TOKEN', function() { return JSON.stringify(process.env.ROLLBAR_CLIENT_TOKEN || null); });
...
app.waitConfiguredAsync = function isConfigured () {
  return isConfiguredPromise;
}

var isConfiguredPromise = new Promise(function(resolve) {
  addon.on('configured', function() {
    module.exports.routes = require('./routes')(app, addon);
    resolve();
  })
});
