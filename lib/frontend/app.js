...
class SidebarJob extends React.Component {
  ...
  render() {
    ...
    const jobVideoDialogOptions = JSON.stringify({ urlTemplateValues: { message: { metadata: { jobId: job.id } } } });
    return (
      <li className={`aui-connect-list-item list-item-sauce-status-${job.consolidated_status}`}>
        ...
        <div className="aui-connect-list-item-description">
          <p>List view descriptions are optional.</p>
          <p><a href='#' onClick={this.showVideo}>Video</a></p>
        </div>
      </li>
    );
  }
